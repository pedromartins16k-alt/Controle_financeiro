...
export function ReportsView({
  ...
  const handleExportCSV = () => {
    ...
    const rows = transactions.map((t) => [
      ...
      `"${t.descricao}"`,
      t.tipo,
      `"${categoryMap.get(t.categoria_id)?.nome || ""}"`,
      t.forma_pagamento || "—",
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-full bg-paper-raised p-1">
            {[
              ...
              <a
                key={p.id}
                href={`/relatorios?periodo=${p.id}`}
                className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
                  selectedPeriod === p.id
                    ? "bg-brand text-brand-foreground"
                    : "text-text-secondary hover:text-text-primary hover:bg-paper-overlay"
                }`}
              >
                {p.label}
              </a>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="p-5">
          ...
          <p className={`mt-2 text-2xl font-bold tracking-tight ${saldoLiquido >= 0 ? "text-income" : "text-expense"}`}>
              ...
              {transactions.map((t) => {
                ...
                return (
                  <tr key={t.id} className="hover:bg-paper-raised/50">
                    ...
                    <td className={`py-3 text-right font-semibold tabular-data ${isIncome ? "text-income" : "text-expense"}`}>
